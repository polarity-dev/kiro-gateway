...
**Population Strategy:**
- Live discovery via the legacy Q `ListAvailableModels` endpoint
- Per-account TTL refresh
- Persisted last-known-good metadata in `state.json`; no static model list
- Display IDs generated by `build_model_display_id()` and exposed by `/v1/models`
- Claude Code's local allowlist synchronized by `scripts/sync_claude_models.py`
